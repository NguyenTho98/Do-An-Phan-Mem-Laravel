                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr class="post{{$category->id}}">
                            ...
                            <td>{{ $category->products->where('is_delete', false)->where('price', '>', 0)->count() }}</td>
                            <td>{{ $category->created_at }}</td>
                            <td class="actions">
                                <a href="{{ url('admin/categories/detail')."/".$category->id }}" class="edit-row"><i class="fas fa-pencil-alt"></i></a>
                                <a href="#modalDelete" class="delete-row modal-with-zoom-anim" data-id="{{ $category->id }}"><i class="far fa-trash-alt"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
